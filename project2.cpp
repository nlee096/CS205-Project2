...
double data[MAX_INSTANCES][MAX_FEATURES];
...
void normalize(){
	...
	for(int i = 1; i < num_features; i++){
		...
		mean[i] = sum / (double)num_instances;
	}
	...
	for(int i = 1; i < num_features; i++){
		...
		std[i] = sqrt(variance/ (double)num_instances);
	}
...
double cross_validation(set<int> &feature_set){
	...
	vector<vector<double> > priorCalc;
	double label;
	double temp;
	double diff;
	set<int>::iterator it;
	...
	for(int i = 0; i < num_instances; i++){
        ...
        //checks array for pairs previously calculated
		if(!priorCalc.empty()){ 
			for(k = 0; k < i; k++){
				temp = priorCalc[k][i-1-k];
				if(temp < nnDist){
					nn = k;
					nnDist = temp;
				}
		...
		vector<double> storeDist;
		for(k = i+1; k < num_instances; k++){
			//if(k == i){continue;}
...
void forward_search(/*no input needed for now*/){
    ...
    cout << "Using no features I get an accuracy of " << best_acc << "%"<< endl;
    ...
    cout << "}, which has an accuracy of " << best_acc << "%" << endl;
    //auto se = high_resolution_clock::now();
    //auto total = duration_cast<microseconds>(se - ss);
    //cout << total.count() << " microseconds" << endl;
}
...
void back_elim(/*no input needed for now*/){
    ...
    cout << "Using all features I get an accuracy of " << best_acc << "%"<< endl;
    //cout << "time : " << aft_time.count() << endl;
    cout << "Beginning search" << endl << endl;
...
int main()
{
    cout << "Welcome to Nathan Lee's Feature Selection Algorithm." << endl;
    cout << "Select which data set:" << endl;
    cout << "1. small-test-dataset.txt" << endl;
    cout << "2. CS170_XXXlarge_Data__1.txt" << endl;
    cout << "3. CS170_small_Data__32.txt" << endl;
    cout << "4. CS170_large_Data__32.txt" << endl;
    int fileOption;
    string fileName;
    cin >> fileOption;
    if(fileOption == 1){
    	...
    	fileName = "CS170_XXXlarge_Data__1.txt";
    }
    ...
    //cout << num_features << " " << instance_num << endl;
    ...
    cout << "Type the number of the algorithm you want to run." << endl
        ...
        << "3 : Nathan's Special algorithm (not implemented. Does nothing)" << endl;
